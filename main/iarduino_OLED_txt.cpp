...
void	iarduino_OLED_txt::clrScr(char color){																			//	Параметр: флаг закрашивания экрана в белый цвет.
...
void	iarduino_OLED_txt::invScr(char invert){																			//	Параметр: флаг инвертирования - да/нет.
			if(invert ==1)	{_sendCommand(SSD1306_INVERT_ON );}																//	Если флаг invert установлен - включаем инверсию.
...
void	iarduino_OLED_txt::setFont(uint8_t* font){																		//	Параметр:  название шрифта.
			...
			insFont.setFont		=	1;																				//	Устанавливаем флаг выбора шрифта.
...
void	iarduino_OLED_txt::_print(char* txt, int col, int str){															//	Параметры: текст, № колонки, № строки (страницы).
			if(insFont.setFont==0 || insFont.height%8>0){return;}													//	Если шрифт не выбран или его высота не кратна 8 пикселям, то выходим из функции.
...
void	iarduino_OLED_txt::_sendCommand(uint8_t command){																//	Параметр: код команды.
			twi_write_byte(dispAddr, SSD1306_COMMAND, command);														//	Отправляем 3 байта: адрес dispAddr с битом rw=0 (запись), SSD1306_COMMAND, command.
}																														//
																														//
...
void	iarduino_OLED_txt::_sendData(uint8_t* data, uint8_t sum){														//	Параметры: указатель на начало массива данных, количество передаваемых байт
			for(uint8_t i=0; i<sum; i+=30){
                twi_write_array(dispAddr,SSD1306_DATA,&data[i],((i+30)<sum)?30:(sum-i));}//	Передаём данные многобайтным пакетом, но не более 30 байт в одном пакете (ограничение буфера Wire.h)
}																														//