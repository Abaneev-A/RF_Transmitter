...
#include <string.h>        																									//																									//																											//																			//	Подключаем файл iarduino_OLED_txt_I2C.h - для работы с шиной I2C        (используя функции производного класса iarduino_I2C)
#include "twi.h"
																											//
...
class iarduino_OLED_txt{																					//
	...
	/**	Пользовательские функции **/																		//
		...
		void				clrScr			(char=0													);	//	Объявляем  функцию очистки дисплея						(Параметр:  флаг закрашивания экрана в белый цвет)
		void				fillScr			(char i=1	){	clrScr(i);								}	//	Определяем функцию заливки дисплея						(Параметр:  флаг закрашивания экрана в белый цвет)
		void				invScr			(char=1													);	//	Объявляем  функцию инвертирования цвета дисплея			(Параметр:  флаг инвертирования - да/нет)
		void				invText			(char i=1	){	insFont.inverted=i;						}	//	Определяем функцию инвертирования цвета текста			(Параметр:  флаг инвертирования - да/нет)
		...
		void				setCoding		(uint8_t i=0){	codingName=i;							}	//	Определяем функцию указывающую тип кодировки для print	(Параметры: название кодировки текста для функции print)
		...
		void				print			(int8_t			,	int=OLED_N,	int=OLED_N, uint8_t=10		);	//	Объявляем  функцию вывода чисел							(Параметры: число, № колонки, № строки, система счисления)
		void				print			(uint8_t		,	int=OLED_N,	int=OLED_N, uint8_t=10		);	//	Объявляем  функцию вывода чисел							(Параметры: число, № колонки, № строки, система счисления)
		void				print			(int16_t		,	int=OLED_N,	int=OLED_N, uint8_t=10		);	//	Объявляем  функцию вывода чисел							(Параметры: число, № колонки, № строки, система счисления)
		void				print			(uint16_t		,	int=OLED_N,	int=OLED_N, uint8_t=10		);	//	Объявляем  функцию вывода чисел							(Параметры: число, № колонки, № строки, система счисления)
		void				print			(int32_t		,	int=OLED_N,	int=OLED_N, uint8_t=10		);	//	Объявляем  функцию вывода чисел							(Параметры: число, № колонки, № строки, система счисления)
		void				print			(uint32_t		,	int=OLED_N,	int=OLED_N, uint8_t=10		);	//	Объявляем  функцию вывода чисел							(Параметры: число, № колонки, № строки, система счисления)
	...
	/**	Внутренние переменные **/																			//
		...
		uint8_t				codingName	=	TXT_WIN1251;														//	Определяем переменную для хранения текущей кодировки.
		...
		struct{																								//	Определяем структуру параметров используемого шрифта:
			...
			char			inverted	=	0;															//	-          флаг указывающий на необходимость вывода шрифта с инвертированием его цвета. Флаг устанавливается/сбрасывается функцией invText().
			char			setFont		=	0;															//	-          флаг указывающий на то, что шрифт выбран.
